...
from .sportsreid import SportsreID
...
def make_dataloader(cfg):
    ...
    #将 dataset.train（训练集）和 train_transforms（训练时的图像预处理）传给 ImageDataset 类，返回一个训练集对象 train_set。
    train_set = ImageDataset(dataset.train, train_transforms)
    #将 dataset.train（训练集）和 val_transforms（验证时的图像预处理）传给 ImageDataset 类，返回一个经过验证集预处理的训练集对象 train_set_normal，用于第二阶段训练
    train_set_normal = ImageDataset(dataset.train, val_transforms)
    #num_classes：训练集中的类别数量（通常是行人或物体的数量）。
    #cam_num：训练集中的摄像头数量。
    #view_num：训练集中的视角数量。
    ...
    #创建一个验证集对象 val_set，它包含查询集（dataset.query）和图库集（dataset.gallery），并对其进行验证时的图像预处理。
    val_set = ImageDataset(dataset.query + dataset.gallery, val_transforms)

    #创建验证集的 DataLoader，并设置 shuffle=False，表示验证集数据不需要打乱顺序。
    val_loader = DataLoader(
        val_set, batch_size=cfg.TEST.IMS_PER_BATCH, shuffle=False, num_workers=num_workers,
        collate_fn=val_collate_fn
    )
    #创建第一阶段训练数据的 DataLoader，train_set_normal 使用的是验证集预处理的训练集。

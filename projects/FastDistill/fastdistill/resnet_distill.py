...
class BasicBlock(nn.Module):
    expansion = 1
    #inplanes:输入的通道数（即前一层的输出通道数）
    #reduction:SE模块中的压缩比例  Squeeze-and-Excitation
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, bn_norm, with_ibn=False, with_se=False,
        ...
        self.bn2 = get_norm(bn_norm, planes)
        #将通道数从 planes 扩展为 planes * self.expansion
...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, block, layers, non_layers):
        self.channel_nums = [] #用于记录每一层的通道数
        self.inplanes = 64  #网络的初始通道数
        super().__init__()
    ...
    def get_bn_before_relu(self): #返回一个包含每一层批归一化层的列表
    ...
    def extract_feature(self, x, preReLU=False): #可以返回不同深度的特征图
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
    ...
    def get_channel_nums(self): #依次保存各个层的通道数
        return self.channel_nums


def init_pretrained_weights(key):#加载一个已经在大数据集上训练好的模型
    ...
    def _get_torch_home(): #该函数用于获取 PyTorch 模型缓存文件的默认存储路径
    ...
    #根据 TORCH_HOME 获取路径后，创建一个名为 checkpoints 的子目录，用于存放预训练模型的权重文件。
    ...
    cached_file = os.path.join(model_dir, filename)#将文件名拼接到 model_dir 路径上，生成模型文件的完整路径
    if not os.path.exists(cached_file):
        if comm.is_main_process():
            gdown.download(model_urls[key], cached_file, quiet=False)

    #用于分布式训练中，确保所有进程都完成了操作，保持同步
    comm.synchronize()
...
def build_resnet_backbone_distill(cfg):
    ...
    with_nl       = cfg.MODEL.BACKBONE.WITH_NL  #Non-local
    depth         = cfg.MODEL.BACKBONE.DEPTH
    # fmt: on

    #这个字典为不同深度的 ResNet 定义了每个阶段（layer1、layer2、layer3、layer4）中的块（block）数
    ...
    #这个字典定义了每个阶段中应该添加多少个 Non-local（NL）模块
    nl_layers_per_stage = {
        '18x': [0, 0, 0, 0],
        '34x': [0, 0, 0, 0],
        '50x': [0, 2, 3, 0],
